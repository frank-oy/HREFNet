...
class EncoderConv(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.gn = nn.GroupNorm(out_ch // 4, out_ch)  #分组1归一化，通常用于小batch的训练当中，out_ch // 4：这是组归一化层的组数
...
class DSCNet(nn.Module):
    def __init__(
        ...
        卷积核是否需要变形，若为false，则为标准的卷积核
        :param device: set on gpu
        :param number: basic layer numbers  基本层的数量
        :param dim: 维度
        """
        ...
        """
        self.dim = dim  # This version dim is set to 1 by default, referring to a group of x-axes and y-axes指的是一组x轴和一组y轴
        """
    ...
    def forward(self, x):
        # block0
        x_00_0 = self.conv00(x)  # 普通卷积
        x_0x_0 = self.conv0x(x)  # 在x轴方向上变换的卷积
        x_0y_0 = self.conv0y(x)  # 在y轴方向上变换的卷积
        x_0_1 = self.conv1(cat([x_00_0, x_0x_0, x_0y_0], dim=1))  #再链接起来
