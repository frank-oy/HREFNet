...
class DSConv(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size, extend_scope, morph,
        ...
        self.offset_conv = nn.Conv2d(in_ch, 2 * kernel_size, 3, padding=1) #返回偏移量 返回图像尺寸大小不变
        ...
        self.dsc_conv_x = nn.Conv2d(
            in_ch,
            out_ch,
            kernel_size=(kernel_size, 1),  #卷积核高度为kernel_size,宽度为1
            stride=(kernel_size, 1),   #每次在高度上移动kernel_size，在水平上移动1
            #这样设计可以在高度方向上进行较大的步幅卷积操作，同时在宽度方向上进行逐步卷积。
            padding=0,
        )
    ...
    def forward(self, f):
        offset = self.offset_conv(f)# [B , 2*k , W , H]
...
class DSC(object):
    ...
    def _coordinate_map_3D(self, offset, if_offset):
        # offset
        y_offset, x_offset = torch.split(offset, self.num_points, dim=1) # y_offset[B,k,W,H]offset的前半部分
        #将偏移量 offset 沿着通道维度分割成 y_offset 和 x_offset，分别表示 y 轴和 x 轴的偏移。
        ...
        #生成最终 y_center 的形状将是 (1, 3, 5, 4)的四维张量图
        ...
        if self.morph == 0:
            ...
            """
            #y与x生成一些值x为【0】，y为[-num_points/2,num_points/2]生成值的个数为num_points
            ...
            y_grid = y_grid.unsqueeze(0)  # [1,B*K*K, W,H]

            x_grid = x_spread.repeat([1, self.width * self.height])
            x_grid = x_grid.reshape([self.num_points, self.width, self.height])
            x_grid = x_grid.unsqueeze(0)  # [1,B*K*K, W,H]
            ...
            if if_offset:
                ...
                center = int(self.num_points // 2)  #图像中心点的索引

                # The center position remains unchanged and the rest of the positions begin to swing
                # This part is quite simple. The main idea is that "offset is an iterative process"
                y_offset_new[center] = 0   #图像中心点偏移量为0
                #摇摆的核心部分：下一个位置的偏移量有前一个位置与现在位置共同决定
