...
class CrossMergeFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, ys: torch.Tensor, raw_weights: torch.Tensor):
        weights = torch.softmax(raw_weights, dim=0)
        B, K, D, H, W = ys.shape  # K=8
        ys = ys.view(B, K, D, -1)  # (B, 8, D, H*W)
        ctx.shape = (B, D, H, W)
        ctx.save_for_backward(ys, weights)

        # 计算水平方向和垂直方向
        y_rb = weights[0] * ys[:, 0] + weights[1] * ys[:, 1]
        y_rb += weights[2] * ys[:, 2].flip(dims=[-1]) + weights[3] * ys[:, 3].flip(dims=[-1])
        y_rb = y_rb.view(B, D, H, W)  # 直接重塑为空间维度

        # 计算对角线方向的两个部分并堆叠
        y_da_part1 = weights[4] * ys[:, 4] + weights[6] * ys[:, 6].flip(dims=[-1])
        y_da_part2 = weights[5] * ys[:, 5] + weights[7] * ys[:, 7].flip(dims=[-1])
        y_da = torch.stack([y_da_part1, y_da_part2], dim=1)  # (B, 2, D, H*W)

        # 分别处理对角线和反对角线
        y_da_diag = diagonal_scatter(y_da[:, 0], (B, D, H, W))
        y_da_anti = antidiagonal_scatter(y_da[:, 1], (B, D, H, W))
        y_da = y_da_diag + y_da_anti

        y_res = y_rb + y_da
        return y_res.view(B, D, -1)  # (B, D, H*W)

    @staticmethod
    def backward(ctx, grad_output: torch.Tensor):
        B, D, H, W = ctx.shape
        ys, weights = ctx.saved_tensors
        grad_output = grad_output.view(B, D, H, W)

        ys_grad = torch.zeros_like(ys)
        weights_grad = torch.zeros_like(weights)

        # 水平/垂直方向梯度
        ys_grad[:, 0] = grad_output.view(B, D, -1) * weights[0]
        ys_grad[:, 1] = grad_output.transpose(2, 3).contiguous().view(B, D, -1) * weights[1]
        ys_grad[:, 2] = torch.flip(ys_grad[:, 0], [-1]) * weights[2]
        ys_grad[:, 3] = torch.flip(ys_grad[:, 1], [-1]) * weights[3]

        # 对角线方向梯度
        diag_grad = diagonal_gather(grad_output)
        anti_grad = antidiagonal_gather(grad_output)
        ys_grad[:, 4] = diag_grad * weights[4]
        ys_grad[:, 5] = anti_grad * weights[5]
        ys_grad[:, 6] = torch.flip(diag_grad, [-1]) * weights[6]
        ys_grad[:, 7] = torch.flip(anti_grad, [-1]) * weights[7]

        # 权重梯度计算
        for i in range(8):
            weights_grad[i] = (ys[:, i] * grad_output.view(B, D, -1)).sum()
        weights_grad = weights * (weights_grad - (weights * weights_grad).sum())

        return ys_grad.view(B, 8, D, H, W), weights_grad

class CrossMerge(nn.Module):
    def __init__(self):
        super().__init__()
        self.raw_weights = nn.Parameter(torch.ones(8))

    def forward(self, ys: torch.Tensor):
        return CrossMergeFunction.apply(ys, self.raw_weights)
